...
class OpenAILegacy:
    ...
    >>> chat_provider = OpenAILegacy(model="gpt-5", api_key="test")
    >>> chat_provider.name
    'openai'
    ...
    def _convert_message(self, message: Message) -> ChatCompletionMessageParam:
        ...
        # Ensure tool_call arguments is always a valid JSON string (not None or "")
        # so that proxies can parse it into a dict for Anthropic's tool_use.input
